...
def backup(inp):
    """ LDC universal document id (May 2017 edition): 9 character sequence at end of underscore-separated filename
    (after stripping any period-type extensions) """
...
def main():
    parser = argparse.ArgumentParser(
        description="Given category per doc, idfile, data file, put data in category-specific dir",
    ...
    parser.add_argument("--remainder", "-r", default="train",
                        help="remainder category. Should match previous remainder category")
    addonoffarg(parser, 'backup', help="backup matches to universal docid, following strict ldc format (in May 2017)",
                default=True)
